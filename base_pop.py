...
class IPU():
	...
	def preprocess_individual_samples_ihds(self, filtered_ihds_individuals_data):
		...
		individuals_data['Age'] = individuals_data['Age'].apply(lambda x : helper.try_convert(x, np.float('nan'), int) )   
		...
		# individuals_data['activity_status'] = individuals_data['activity_status'].apply(lambda x : helper.try_convert(x, np.float('nan'), int) )   
